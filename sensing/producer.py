...
class Generator:
    ...
    async def gen_blood_pressure(self):
        ...
        while True:
            ...
            json_text = {'id': 2, 'systolic': float(systolic), 'diastolic': float(diastolic)}
            self.channel.basic_publish(exchange='logs', routing_key='', body= json.dumps(json_text))
            await asyncio.sleep(2)
    ...
    async def gen_body_temp(self):
        ...
        while True:
            ...
            json_text = {'id': 3, 'temperature': float(temperature)}
            self.channel.basic_publish(exchange='logs', routing_key='', body= json.dumps(json_text))
            await asyncio.sleep(2)
    ...
    async def gen_sugar_level(self):
        ...
        while True:
            ...
            json_text = {'id': 4, 'after': float(after), 'before': float(before)}
            self.channel.basic_publish(exchange='logs', routing_key='', body= json.dumps(json_text))
            await asyncio.sleep(2)
